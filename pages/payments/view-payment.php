...
$stmt = $pdo->prepare("
    SELECT r.*, i.invoice_number, i.grand_total as invoice_total, r.status
    FROM receipts r
...
<body class="bg-gray-50 text-gray-900">
    ...
    <script>
        ...
        async function deletePayment(id) {
            if (!confirm("CRITICAL WARNING:\n\nAre you sure you want to DELETE this entire payment?\n\n- All linked receipts will be deleted.\n- Any invoice balances paid by this will be reversed (they will become unpaid).\n- Any customer credit generated by this payment will be deducted.\n\nThis action cannot be undone.")) {
                return;
            }

            const reason = prompt("Optional: Reason for deleting payment?");

            try {
                const res = await fetch('../../api/payments/delete-payment.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ payment_id: id, reason: reason })
                });
                const data = await res.json();

                if (data.success) {
                    alert('Payment deleted successfully.');
                    window.location.href = 'manage-payments.php';
                } else {
                    alert('Error: ' + data.message);
                }
            } catch (e) {
